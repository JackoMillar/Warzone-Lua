...
function PurchaseClicked()
	...
	for _, order in pairs(Game.Orders) do
		if order.proxyType == 'GameOrderCustom' and startsWith(order.Payload, 'BuyRecruiter_') then
			numRecruitersAlreadyHave = numRecruitersAlreadyHave + 1;
		end
	end

	if numRecruitersAlreadyHave >= Mod.Settings.MaxRecruiters then
		UI.Alert('You already have ' .. numRecruitersAlreadyHave .. ' Recruiters! You can only have ' ..  Mod.Settings.MaxRecruiters);
...
function PresentBuyRecruitersDialog(rootParent, setMaxSize, setScrollable, game, close)
	...
	SelectTerritoryBtn = UI.CreateButton(vert)
		.SetText('Select Territory')
		.SetOnClick(SelectTerritoryClicked);

	TargetTerritoryInstructionLabel = UI.CreateLabel(vert).SetText('');

	BuyRecruiterBtn = UI.CreateButton(vert)
		.SetInteractable(false)
		.SetText('Complete Purchase')
		.SetOnClick(CompletePurchaseClicked);
...
function TerritoryClicked(terrDetails)
	...
	if not terrDetails then
		...
		BuyRecruiterBtn.SetInteractable(false);
	...
	if Game.LatestStanding.Territories[terrDetails.ID].OwnerPlayerID ~= Game.Us.ID then
		TargetTerritoryInstructionLabel.SetText('You may only receive a Recruiter on a territory you own.  Please try again.');
	else
		TargetTerritoryInstructionLabel.SetText('Selected territory: ' .. terrDetails.Name);
		SelectedTerritory = terrDetails;
		BuyRecruiterBtn.SetInteractable(true);
	end
end

function CompletePurchaseClicked()
	local msg = 'Buy a Recruiter on ' .. SelectedTerritory.Name;
