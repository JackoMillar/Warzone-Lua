function Client_PresentSettingsUI(rootParent, game)
	...
	if Mod.Settings.OnlyBaseNeutrals then
	...
	if not Mod.Settings.ONeutrals then
	...
	if not Mod.Settings.FixedArmies then
		UI.CreateLabel(vert).SetText('Random +/- limit of armies: ' .. tostring(Mod.Settings.aLuck)).SetColor('#09cc02');
	end
	...
	if not Mod.Settings.FixedPieces then
		UI.CreateLabel(vert).SetText('Random +/- limit of pieces: ' .. tostring(Mod.Settings.rLuck)).SetColor('#FFD700');
	end

	--misc settings
	if not Mod.Settings.AttackNeutral then
